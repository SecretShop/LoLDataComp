...
def print_Stuff(cursor, table_name):
    # prints stuff
    
    c.execute('SELECT GAMEID, CHAMP FROM {}'.format(table_name))
    print("GAME ID, CHAMP")
    stuff = c.fetchall()
    #print(stuff)
    for k in range(1,len(stuff)):
        print("{}) {}, {}".format(k,stuff[k][0],stuff[k][1]))
    print("Total games: {}".format(len(stuff)))


if __name__ == '__main__':
 
    sqlite_file = 'champs.db'
    table_name = 'RANKED'

    conn, c = connect(sqlite_file)
    print_Stuff(c,table_name)
